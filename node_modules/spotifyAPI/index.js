...
module.exports = {
    ...
    updateUser: function(req, res) {
        return fetch('https://api.spotify.com/v1/me', {
            ...
            req.session.username = profileData.display_name;
            if(profileData.images && profileData.images.length > 0) {
                req.session.pfp = profileData.images[0].url;
            } else {
                req.session.pfp = "/images/noPfp.jpg";
            }

            if (account.userExists(req.session)) {
                account.updateRefreshToken(req.session);
            } else {
                account.newUser(req.session);
            }
        }).catch(function(err) {
            console.error(err);
            throw err;
